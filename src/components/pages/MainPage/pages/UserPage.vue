<template>
  <div class="container column" style="position: relative">
    <TopBar class="topBar" style="position: relative">
      <div class="row" style="margin: 0.9375rem 1.25rem; justify-content: space-between">
        <div class="switchButton row">
          <img :src="require('@/assets/svg/switch_line.svg')" />
          <span @click="$router.push({ name: 'IdentitySwitchPage' })">切换为人才</span>
        </div>
    ...
    <div class="view column" style="position: absolute; top: 6.75rem">
      <!-- 用户信息框 -->
      <div v-loading="!user" class="userInfoBox column" style="padding: 2.5rem 0.625rem 0.625rem 0.625rem; position: relative">
        <img style="position: absolute; top: -1.75rem" class="avatar" :src="tryGet(user, 'avatar')" alt="" />
        <div class="primary">{{ tryGet(user, 'name') }}</div>
        <TagBox style="margin-top: 0.3125rem" :tags="tryGet(user, 'tags')" :editable="true" />
        <div style="margin-top: 0.625rem" class="row">
          <span class="secondary">综合评分：</span>
          <ScoreBox :score="tryGet(user, 'score')" />
        </div>
      </div>
      <OperationBar style="width: 100%; margin-top: 15px" :datas="tryGet(datas, 'rowButtons')" />
      <div style="width: 100%; margin-top: 15px">
        <OperationItem v-for="(item, key) in tryGet(datas, 'options')" :key="key" :data="item" @click="$router.push({ name: item.routeName })" />
      </div>
    </div>
  </div>
</template>
