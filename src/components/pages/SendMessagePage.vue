<template>
  <div class="container">
    <TopBar class="topBar row" style="justify-content: space-between">
      ...
      <img style="margin-right: 13px" :src="require('@/assets/svg/person_line.svg')" alt="" />
    </TopBar>

    <div class="view" v-loading="!dialogue">
      <MessageBubble v-for="(item, key) in tryGet(dialogue, 'messages')" :key="key" :dir="item.sender.id === $route.params.id ? 'left' : 'right'" :text="item.text" :user="item.sender" />
    </div>

    <div class="bottomBar">
      <input type="text" v-model="input" placeholder="发送消息" />
      <el-button style="margin-left: 10px" type="primary" @click="handleSend">发送</el-button>
    </div>
  </div>
</template>
